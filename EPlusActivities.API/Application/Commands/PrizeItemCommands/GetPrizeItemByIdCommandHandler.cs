...
{
    public class GetPrizeItemByIdCommandHandler : BaseCommandHandler, IRequestHandler<GetPrizeItemByIdCommand, PrizeItemDto>
    {
        public GetPrizeItemByIdCommandHandler(UserManager<ApplicationUser> userManager, IPrizeItemRepository prizeItemRepository, INameExistsRepository<Brand> brandRepository, INameExistsRepository<Category> categoryRepository, IMapper mapper) : base(userManager, prizeItemRepository, brandRepository, categoryRepository, mapper)
        {
        }

        public async Task<PrizeItemDto> Handle(GetPrizeItemByIdCommand request, CancellationToken cancellationToken)
        {
